...
def generate(pairing, g1, g, p, input_context, model, users, userr, l):
    σ, h = crypto.sign_bls(pairing, g1, g, p, input_context, users[0], l)
    s = σ
    ss = list(s)
    n = 0
    words = []
    ...
    for i in range(4*l*2):
        ...
        if n == l:
            m1 = "".join(words)
            σ1 = crypto.sign_bls(pairing, g1, g, p, m1, users[0], l)
            print("σ1 == ", σ1)
            s1 = σ1[0]
            ss.extend(s1)
            words = []
...
def verify(pairing, input_context, output_context, g1, pks, l, p, strd, wordss, target_len=5):
    ...
    # print("s == ",s," x == ",x)
    m1 = m1
    h = crypto.hash(pairing, g1, p, m1)
    print("The verify result is：", crypto.verify_bls(pairing, h, pks, s, l))
...
def main():
    ...
    now3 = []
    for i in range(1):
        output_context, strd, wordss = generate(pairing, g1, g, p, input_context, model, users, userr, l)
        now1 = datetime.datetime.now()
        verify(pairing, input_context, output_context, g1, users[1], l, p, strd, wordss)
        now2 = datetime.datetime.now()
        #now3.append(now2-now)
    #now2 = datetime.datetime.now()
    print("The average duration over 1 times is：", (now1 - now) / 1)
    print("The verification time is：", now2-now1)
    # verify(pairing, input_context, output_context, g1, users[1], l, p)
